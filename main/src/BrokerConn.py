...
class BrokerConn:
    ...
    def create_trade(self, Stock: str, Amount: float) -> dict:
        ...
        if estimatedCost > self.get_balance():
            raise InsuffecientFunds(self.get_balance(), Amount)
    ...
    def leave_trade(self, Stock: str, Amount: float) -> dict:
        try:
            ...
            while True:
                ...
                if newOrder.status == "filled" or newOrder.status == "new":
                    break
                time.sleep(1)  # Wait for a second before checking again
            ...
            # Handle value errors (e.g., stock not in portfolio, insufficient shares)
    ...
    def get_info(self, Stock: str, time = None) -> dict:
        ...
        try:
            ...
            if not barset:
                raise ValueError(f"No data available for stock symbol '{Stock}' at time '{time}'")
...
print(trader.get_info("AAPL", time = "2024-07-01 16:14:08"))
print(trader.create_trade("AAPL", 1))
